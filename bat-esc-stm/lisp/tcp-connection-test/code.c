...
static void stop(void *arg) {
  ...
  if (!d) VESC_IF->printf("d is NULL");
  else if (!d->thread) VESC_IF->printf("thread is NULL");
  else {
...
static bool restore_at_if(void) {
  char linebuffer[20];
  int br;
  while (VESC_IF->uart_read() >= 0);  // purge
  ...
  if (br > 0) {
    if (strneql(response, linebuffer, strlen(response))) {
      ...
      if (n_bytes > 0) {
        ...
        int bytes_read = tcp_read_data((unsigned char *)linebuffer, size, n_bytes);
        if (bytes_read == n_bytes) {
          for (int i = 0; i < bytes_read; i ++) {
            VESC_IF->packet_process_byte((unsigned char)linebuffer[i], d->p_state);
          }
        }
      }
      br = uart_read_until(linebuffer, "\n", size);
  ...
  while (!VESC_IF->should_terminate()) {
    ...
    if (d->cmd) {
      ...
      case COMMAND_PAUSE:
        d->paused = true;
        restore_at_if();
        VESC_IF->lbm_unblock_ctx_unboxed(d->cmd_thread, VESC_IF->lbm_enc_sym_true);
        d->cmd_thread = -1;
        d->cmd = 0;
        break;
...
INIT_FUN(lib_info *info) {
  ...
  unsigned char *buffer  = VESC_IF->malloc(SEND_BUFFER_UNITS * sizeof(send_unit_t));
  if (!buffer) {
    VESC_IF->free(d);
    return false;
  }
