...
typedef struct {
    ...
    lbm_uint symbol_false_v;
    lbm_uint symbol_null;
    lbm_uint symbol_plus_assoc;

    lbm_uint symbol_undetermined;
...
static bool register_symbols() {
    ...
    bool res =
        ...
        && VESC_IF->lbm_add_symbol_const("false_v", &d->symbol_false_v)
        && VESC_IF->lbm_add_symbol_const("null", &d->symbol_null)
        && VESC_IF->lbm_add_symbol_const("+assoc", &d->symbol_plus_assoc)
        && VESC_IF->lbm_add_symbol_const("undetermined", &d->symbol_undetermined)
...
static ssize_t str_index_of(const char *str, const char *search) {
    ...
    for (size_t i = 0; i < len - (search_len - 1); i++) {
...
/**
 * Helper function for json-stringification functions.
 *
 * \param buffer The buffer to copy str to. If null, nothing is written and the
 * length of `str` is returned.
 * Must be large enough to hold the entirety of `str` plus the terminating null
 * byte.
 * \return The amount of bytes that were written or would have been
 * written.
 */
static size_t json_maybe_copy_str(char *buffer, const char *str) {
    size_t len = strlen(str);
    if (buffer) {
...
static size_t json_stringify_object(char *dest, const lbm_value json_object) {
    ...
    offset += json_maybe_copy_str(dest + offset, "{");
    ...
    for (size_t i = 0; i < 200; i++) {  // safeguard
        ...
        offset += json_maybe_copy_str(dest + offset, name);
...
static size_t json_stringify_str(char *dest, const char *str) {
    ...
    offset += json_maybe_copy_str(dest + offset, "\"");
    offset += json_maybe_copy_str(dest + offset, str);
    offset += json_maybe_copy_str(dest + offset, "\"");

    return offset;
}

static size_t json_stringify_simple_value(
    char *dest, const lbm_value json_value
) {
    data *d = use_state();

    if (VESC_IF->lbm_is_symbol(json_value)) {
        lbm_uint symbol = VESC_IF->lbm_dec_sym(json_value);

        if (VESC_IF->lbm_is_symbol_true(symbol)) {
            return json_maybe_copy_str(dest, "true");
        } else if (symbol == d->symbol_false_v) {
            return json_maybe_copy_str(dest, "false");
        } else if (symbol == d->symbol_null) {
...
static size_t json_stringify(char *dest, const lbm_value json_value) {
    ...
    if (VESC_IF->lbm_is_symbol_nil(json_value)) {
        ...
        if (lbm_is_specific_symbol(first, d->symbol_plus_assoc)) {
            ...
            return json_stringify_array(dest, json_value);
        }
    } else if (VESC_IF->lbm_is_byte_array(json_value)) {
        const char *str = VESC_IF->lbm_dec_str(json_value);
        return json_stringify_str(dest, str);
    } else {
        return json_stringify_simple_value(dest, json_value);
    }
}
...
static size_t str_whitespace_len(const char *str) {
...
static json_lex_unit_t json_lex_str(const char *str) {
    size_t count = str_whitespace_len(str);
    str += count;
...
/**
 * @return If the tokenizer was not able to determine the result with 100%
 * certainty, i.e. it encountered a value that could have been cut in half, then
 * the error field is set to JSON_UNDETERMINED and the found value is set in the
 * value field.
*/
static json_lex_unit_t json_lex_number(const char *str) {
    ...
    if (number_len == len) {
        return (json_lex_unit_t){
            .token = value_lbm,
            .len = count + number_len,
            .error = JSON_UNDETERMINED,
        };
    }
...
static json_lex_unit_t json_lex_syntax(const char *str) {
    ...
    size_t len = str_whitespace_len(str);
 ...
 * tokens.
 * @return struct with the amount of characters consumed. Error field is
...
static json_lex_unit_t json_tokenize_step(const char *str, lbm_value *tokens) {
...

    {
        ...
        if (result.token != VESC_IF->lbm_enc_sym_nil) {
            *tokens = VESC_IF->lbm_cons(result.token, *tokens);
            // if (*tokens == VESC_IF->lbm_enc_sym_merror) {
            //     return *tokens;
            // }
        }
        str = str + result.len;
...

    {
        ...
        if (result.error == JSON_UNDETERMINED) {
            consumed += result.len;
            *tokens = VESC_IF->lbm_cons(result.token, *tokens);
            result.token = VESC_IF->lbm_enc_sym_nil;
            return result;
        ...
        if (result.token != VESC_IF->lbm_enc_sym_nil) {
            *tokens = VESC_IF->lbm_cons(result.token, *tokens);
            // TODO: Check for lbm_enc_sym_merror here!!!
        }
        str = str + result.len;
        consumed += result.len;
    }
...
static lbm_value ext_json_tokenize_step(lbm_value *args, lbm_uint argn) {
    ...
    if (undetermined) {
        tokens = VESC_IF->lbm_cons(VESC_IF->lbm_enc_sym(d->symbol_undetermined), tokens);
        if (tokens == VESC_IF->lbm_enc_sym_merror) {
            return VESC_IF->lbm_enc_sym_merror;
        }
    }
    ...
    lbm_value list = VESC_IF->lbm_cons(len, VESC_IF->lbm_enc_sym_nil);
    if (list == VESC_IF->lbm_enc_sym_merror) {
        return list;
    }
    list = VESC_IF->lbm_cons(result_str, list);
    if (list == VESC_IF->lbm_enc_sym_merror) {
        return list;
    }
    return VESC_IF->lbm_cons(tokens, list);
}
